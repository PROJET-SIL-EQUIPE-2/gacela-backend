...
const addVehicle = async (req, carType, mileage, matricule) => {
  try {
...
const search = async (type, departLat, departLong, destLat, destLong) => {
  try {
    ...
    let closestIdx = await geo.findClosestCar(
      { lat, long },
      carsWithCurrentLocation
    );
    let closest;
    let estimatedPrice;
    if (closestIdx >= 0) {
      closest = cars[closestIdx];
      estimatedPrice = await estimationService.calculateEstimatedPrice(
        type,
        departLat,
        departLong,
        destLat,
